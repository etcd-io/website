---
title: Downgrade etcd from 3.6 to 3.5
weight: 6650
description: Processes, checklists, and notes on downgrading etcd from 3.6 to 3.5
---
...
Highlighted breaking changes from 3.6 to 3.5:

#### Difference in flags

If you are using any of the following flags in your 3.6 configurations, make sure to remove, rename, or change the default value when downgrading to 3.5.

**NOTE** The diff is based on version 3.6.0 and v.3.5.18. The actual diff would be dependent on your patch version, check with `diff <(etcd-3.6/bin/etcd -h | grep \\-\\-) <(etcd-3.5/bin/etcd -h | grep \\-\\-)` first.

```diff
# flags not available in 3.5
...
# equivalent flags of 3.6 feature gates
...
Before beginning, download the latest release of etcd 3.5.
...
This example shows how to downgrade a 3-member 3.6 etcd cluster running on a local machine.

#### Step 1: check downgrade requirements

Is the cluster healthy and running 3.6.x?
...
- We only support downgrading one minor version at a time. e.g downgrading from 3.6 to 3.4 isn't allowed.
...
After enabling downgrade, the cluster will start to operate with 3.5 protocol, which is the downgrade target version. In addition, etcd will automatically migrate the schema to the downgrade target version, which usually happens very fast. Confirm the storage version of all servers has been migrated to 3.5 by checking the endpoint status before moving on to the next step.
...
**NOTE:** Once downgrade is enabled, the cluster will remain operating with 3.5 protocol even if all the servers are still running the v3.6 binary, unless the downgrade is canceled with `etcdctl downgrade cancel`
...
#### Step 6: restart the etcd server with same configuration (minus the flags that are removed or replaced in 3.5)
...
Verify that each member, and then the entire cluster, becomes healthy with the new 3.5 etcd binary:
...
**NOTE:** You will see the `DOWNGRADE ENABLED` is false for the 3.5 server, because the downgrade info is not implemented in 3.5 status endpoint, downgrade is still enabled for the cluster at this point.

#### Step 7: repeat *step 5* and *step 6* for rest of the members

When all members are downgraded, check the health and status of the cluster, and confirm the minor version of all members is 3.5, and storage version is empty:
