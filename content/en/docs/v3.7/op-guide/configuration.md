...
--feature-gates=LeaseCheckpoint=true
  Persists lease remaining TTL to avoid indefinite auto-renewal.
  Replaces: --experimental-enable-lease-checkpoint
--feature-gates=LeaseCheckpointPersist=true
  Persists lease TTLs for compatibility with clusters upgraded from v3.5.
...
--feature-gates=StopGRPCServiceOnDefrag=true
  Stops gRPC service during defragmentation to improve performance and safety.
  Replaces: --experimental-stop-grpc-service-on-defrag
--feature-gates=TxnModeWriteWithSharedBuffer=true
  Enables use of shared buffer for readonly transaction checks.
